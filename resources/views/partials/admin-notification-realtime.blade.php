  ...
  <script>
    (function () {
      ...
      function iconFor(type) {
        ...
        return { icon: 'info', wrap: 'bg-blue-50 dark:bg-blue-500/10', color: 'text-blue-500' };
      }
        ...
        body.innerHTML = notifCache.map(n => {
          ...
          return `
            <div class="px-4 py-3 border-b flex items-start gap-3 cursor-pointer transition-colors notif-row ${isDark ? 'border-slate-800 hover:bg-slate-800' : 'border-slate-50 hover:bg-slate-50'} ${n.read ? 'opacity-55' : ''}" data-id="${n.id}">
              ...
              ${!n.read ? '<span class="w-2 h-2 bg-blue-600 rounded-full flex-shrink-0 mt-2 pulse-dot"></span>' : ''}
            </div>`;
        }).join('');
      ...
      function showNotifToast(title, body) {
        ...
        toast.className = 'toast pointer-events-auto flex items-start gap-3 px-5 py-3.5 rounded-2xl shadow-xl text-sm font-bold bg-blue-600 text-white max-w-xs';
      ...
      async function clearAllNotifs() {
        ...
        try {
          await fetch('{{ route('admin.notifications.clear') }}', {
            method: 'POST',
            headers: { 'X-CSRF-TOKEN': CSRF_TOKEN, 'Accept': 'application/json' }
          });
