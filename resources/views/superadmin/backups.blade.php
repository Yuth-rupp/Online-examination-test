...
<head>
    ...
    <title>Database & Backup — ExamSystem</title>
    ...
    <style>
        ...
        /* Confirm phrase input shake on wrong input */
        @keyframes shake{0%,100%{transform:translateX(0)}25%{transform:translateX(-5px)}75%{transform:translateX(5px)}}
        .shake{animation:shake 0.3s ease-in-out;}

        /* Backup spinning icon */
        ...
        <header class="sticky top-0 z-10 border-b border-slate-100 h-16 flex items-center px-8 gap-4"
            ...
            <div>
                ...
                <p class="text-[11px] text-slate-400 font-medium mt-0.5">
                    Snapshot history · manual backups · point-in-time restoration
                </p>
            </div>
            ...
            <div class="flex items-center gap-3 ml-auto flex-wrap">
                ...
                <div class="flex items-center gap-2 text-xs font-semibold bg-slate-100 text-slate-600 border border-slate-200 px-3 py-1.5 rounded-lg">
                    <span class="relative flex" style="width:8px;height:8px;">
                        <span class="ping-slow absolute inline-flex rounded-full bg-slate-400 opacity-60" style="width:100%;height:100%;"></span>
                        <span class="relative inline-flex rounded-full bg-slate-500" style="width:8px;height:8px;"></span>
                    </span>
                    Syncing <span id="poll-countdown" class="font-mono font-black ml-1 mr-0.5 text-slate-800 w-3 text-center">5</span>s
                </div>
        ...
        <div class="p-8 flex-1" style="display:flex;flex-direction:column;gap:20px;">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div class="bg-white rounded-2xl border border-slate-100 p-5 cursor-default transition-all duration-300"
                    ...
                    <p id="storage-label" class="text-[10px] font-medium mt-1">
                        {{-- Colored by JS --}}
                    </p>
                </div>
            ...
            <div class="bg-white rounded-2xl border border-slate-100 overflow-hidden"
                ...
                <div class="flex items-center justify-between px-6 py-3.5 border-t border-slate-100 bg-slate-50">
                    ...
                    <p class="text-[11px] text-slate-400 font-medium">Auto-refreshes every 5 seconds</p>
                </div>
            </div>
...
{{-- ===================== BACKUP CONFIRM MODAL (low friction) ===================== --}}
...
{{-- ===================== RESTORE MODAL (HIGH FRICTION — 3 deliberate steps) ===================== --}}
    ...
    <div class="modal-in bg-white rounded-2xl max-w-lg w-full border border-rose-200"
        ...
        <div id="restore-step2" style="display:none;" class="px-6 pb-6 pt-4">
            ...
            <p class="text-[11px] text-slate-500 font-medium mb-4 leading-relaxed">
                To confirm you are authorizing an irreversible database overwrite, type the exact phrase below:
            </p>
            ...
            <input id="restore-confirm-input" type="text"
                   placeholder="Type RESTORE here..."
                   oninput="checkRestorePhrase()"
                   autocomplete="off"
            ...
            <div class="flex gap-3">
                ...
                <button id="restore-proceed-btn" onclick="goRestoreStep3()"
                        disabled
        ...
        <div id="restore-step3" style="display:none;" class="px-6 pb-6 pt-4">
            ...
            <p class="text-[11px] text-slate-500 font-medium mb-4 leading-relaxed text-center">
                This action will be logged as a <strong class="text-rose-700">CRITICAL</strong> event in the Forensic Audit Trail under your account.
            </p>
            <div class="flex gap-3">
                <button onclick="closeRestoreModal()"
                        class="flex-1 py-2.5 rounded-xl border border-slate-200 text-sm font-semibold text-slate-600 hover:bg-slate-50 transition-all">
...
// Blade-injected initial snapshots
...
setInterval(()=>{
    pollCount--;
    if(pollCount<=0){ pollCount=5; fetchBackupTelemetry(); }
...
function renderSnapshots(){
    ...
    // Use mock data if blade returned nothing
    const data=snap.length>0?snap:mockSnapshots();
    ...
    if(!data.length){
        tbody.innerHTML=`<tr><td colspan="6" style="padding:48px;text-align:center;color:#94a3b8;">
            ...
            <p style="font-size:12px;font-weight:600;">No snapshots compiled yet.</p>
            </td></tr>`;
        return;
    }
...
// ============================================================
//  MOCK SNAPSHOTS (remove when API is wired)
// ============================================================
function mockSnapshots(){
    return [
        {id:'SNAP-2026-07-15-A',created_at:'2026-07-15 07:33:46',size_mb:14.2,type:'automated'},
        {id:'SNAP-2026-07-14-M',created_at:'2026-07-14 11:33:46',size_mb:13.8,type:'manual'},
        {id:'SNAP-2026-07-13-A',created_at:'2026-07-13 03:00:00',size_mb:13.5,type:'automated'},
        {id:'SNAP-2026-07-12-A',created_at:'2026-07-12 03:00:00',size_mb:12.9,type:'automated'},
    ];
}


// ============================================================
//  API: FETCH BACKUP TELEMETRY
// ============================================================
...
async function executeManualBackup(){
    ...
    try{
        ...
        if(res.ok){
            showToast('<i class="fa-solid fa-circle-check" style="color:#34d399;font-size:13px;"></i><span style="margin-left:8px;">Snapshot created and logged to Audit Trail.</span>');
            setTimeout(()=>fetchBackupTelemetry(),1500);
        } else {
            showToast('<i class="fa-solid fa-circle-xmark" style="color:#f87171;font-size:13px;"></i><span style="margin-left:8px;">Snapshot creation failed.</span>');
        }
    }catch(e){
...
async function executeRestore(){
    ...
    document.getElementById('restore-btn-text').textContent='Writing pools...';
    btn.disabled=true;
    ...
    try{
        ...
        if(res.ok){
            closeRestoreModal();
            showToast('<i class="fa-solid fa-clock-rotate-left" style="color:#34d399;font-size:13px;"></i><span style="margin-left:8px;">Database restored and logged as CRITICAL in Audit Trail.</span>');
            fetchBackupTelemetry();
        } else {
            document.getElementById('restore-btn-text').textContent='Authorize Restoration';
            btn.disabled=false;
            showToast('<i class="fa-solid fa-circle-xmark" style="color:#f87171;font-size:13px;"></i><span style="margin-left:8px;">Restoration failed. Check server logs.</span>');
        }
    }catch(e){
        document.getElementById('restore-btn-text').textContent='Authorize Restoration';
        btn.disabled=false;
        showToast('<i class="fa-solid fa-circle-xmark" style="color:#f87171;font-size:13px;"></i><span style="margin-left:8px;">Network error during restoration.</span>');
    }
}
...
function showToast(html){
    ...
    t.style.cssText=`position:fixed;bottom:24px;right:24px;z-index:9999;display:flex;align-items:center;
        ...
        box-shadow:0 16px 48px rgba(0,0,0,0.35);max-width:380px;
